...
describe("database recovery safeguards", () => {
  ...
  it("publishes encrypted output and restores verified bytes through the drill gate", async () => {
    ...
    const mkdirResult = spawnSync("mkdir", ["-p", binaryDirectory, backupDirectory, restoreTempDirectory]);
    expect(mkdirResult.status).toBe(0);
    const syntheticDump = "synthetic-postgres-custom-dump";
    const pgDumpPath = join(binaryDirectory, "pg_dump");
    const pgRestorePath = join(binaryDirectory, "pg_restore");
    await writeFile(pgDumpPath, `#!/bin/sh\nprintf '${syntheticDump}'\n`);
    await writeFile(pgRestorePath, "#!/bin/sh\nfor last do :; done\ncp \"$last\" \"$RESTORE_CAPTURE_PATH\"\n");
    await chmod(pgDumpPath, 0o700);
    await chmod(pgRestorePath, 0o700);
