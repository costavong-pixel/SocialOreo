...
async function main() {
  ...
  try {
    ...
    const pgRestoreCommand = process.env.SOCIALOLLA_PG_RESTORE_COMMAND ?? "pg_restore";
    const pgRestoreArgv = parseCommandArgv("SOCIALOLLA_PG_RESTORE_ARGV");
    const child = spawn(pgRestoreCommand, [
      ...pgRestoreArgv,
      "--clean",
      "--if-exists",
      "--no-owner",
