...
async function main(): Promise<void> {
  ...
  try {
    ...
    process.stdout.write(`${JSON.stringify({ environment: "staging", email: updated.email, roleBefore, roleDuringTest: updated.role, workspaceId: workspace.id })}\n`);
  } finally {
...
void main().catch((error: unknown) => {
  process.stderr.write(`${error instanceof Error ? error.message : "Staging identity repair failed"}\n`);
  process.exitCode = 1;
});
