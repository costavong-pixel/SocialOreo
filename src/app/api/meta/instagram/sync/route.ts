...
function redirectToAnalysis(request: NextRequest, state: string) {
  return NextResponse.redirect(new URL(`/analysis?instagram=${state}`, request.url), { status: 303 });
}
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    if (isAuthIdentityCollisionError(error)) return redirectToAnalysis(request, "identity_conflict");
    throw error;
  }
  const connection = await prisma.instagramInsightsConnection.findUnique({ where: { userId: user.id } });
  if (!connection || connection.status !== "CONNECTED") return redirectToAnalysis(request, "reconnect");
  try {
    ...
    return redirectToAnalysis(request, "synced");
  } catch {
    ...
    return redirectToAnalysis(request, "sync_failed");
  }
}
