        ...
        <div className="grid gap-5 py-5 lg:grid-cols-[190px_minmax(0,1fr)]">
          <aside className="rounded-xl border border-white/10 bg-[#101a32] p-3 lg:min-h-[1040px]">
            ...
            <div className="mt-8 rounded-lg border border-[var(--social-lime)]/25 bg-[var(--social-lime)]/[0.08] p-3">
              <p className="text-xs font-black uppercase tracking-wide text-[var(--social-lime)]">SocialOlla advantage</p>
...
function TrendMovementPanel({ sources }: { sources: TrendMovementSource[] }) {
  return <section className="mt-5 rounded-xl border border-white/10 bg-[#11141a] p-5">
    <div className="flex flex-col justify-between gap-3 sm:flex-row sm:items-start"><div><p className="text-xs font-bold uppercase tracking-[0.14em] text-orange-300">Observed movement</p><h2 className="mt-1 text-lg font-black">When a pattern earns a trend label</h2><p className="mt-2 max-w-3xl text-sm leading-6 text-white/60">SocialOlla requires at least two completed captures and a recurring public source before it compares movement. This prevents a single capture from being presented as velocity or a trend.</p></div><span className="w-fit rounded-md border border-orange-300/25 bg-orange-400/[0.08] px-2 py-1 text-xs font-black text-orange-200">Evidence gate</span></div>
    ...
    <p className="mt-4 text-xs leading-5 text-white/40">Even after this gate is met, SocialOlla compares only returned public samples and recurring source links. It does not infer platform-wide trend velocity or private Instagram Insights.</p>
  </section>;
}
...
function ObservedHistory({ auditId, monitor }: { auditId: string; monitor: HistoryMonitor | null }) {
  ...
  return <section className="mt-5 rounded-xl border border-white/10 bg-[#11141a] p-5">
    ...
    {monitor?.lastError ? <p className="mt-3 text-xs leading-5 text-amber-100/75">Latest scheduled refresh could not complete. SocialOlla will retry; the stored public baseline remains unchanged.</p> : null}
  </section>;
}
