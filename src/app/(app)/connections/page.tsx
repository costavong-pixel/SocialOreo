            ...
            {destinations.map((destination) => {
              ...
              return (
                <div key={destination.id} className="rounded-2xl border border-white/5 p-4">
                  ...
                  {destination.status === "DISCONNECTED" && !publishingDisabled && <Link href="/api/meta/instagram/publish/connect" className="mt-2 inline-block text-sm font-bold text-amber-200">Reconnect Instagram</Link>}
                  {!destination.providerDisabled && destination.status === "CONNECTED" && (
                    <form action={m2DisconnectInstagramDestination} className="mt-3">
