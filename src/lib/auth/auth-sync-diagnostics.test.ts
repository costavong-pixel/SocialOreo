...
describe("auth sync diagnostics", () => {
  ...
  it("logs only hashes and allow-listed auth state in staging", () => {
    ...
    expect(output).toContain('"callbackResult":"session-input"');
    expect(output).toContain('"connectionProvider":"google-oauth2"');
    expect(output).toContain('"emailVerified":false');
    expect(output).not.toContain("google-oauth2|private-subject");
