...
export function ComparisonHookIdeas({ auditId, competitorId, extractions }: { auditId: string; competitorId: string; extractions: CompetitorHookExtraction[] }) {
  ...
  return (
    <section className="rounded-lg border border-violet-200/20 bg-violet-300/[0.06] p-5 sm:p-7">
      ...
      <div className="mt-5 grid gap-3">{extractions.map((hook, index) => {
        const sourceUrl = safeHttpsUrl(hook.sourceUrl);
        return <article className="rounded-md border border-violet-100/10 bg-black/10 p-4" key={`${hook.sourceUrl}-${index}`}><div className="flex flex-wrap items-center justify-between gap-2"><p className="text-xs font-bold uppercase text-violet-200/70">What they are doing: {hook.pattern.replace(" hooks", "")}</p>{sourceUrl ? <a className="text-xs font-semibold text-violet-100 underline underline-offset-4 hover:text-white" href={sourceUrl} rel="noreferrer" target="_blank">Open their reel</a> : <span className="text-xs text-violet-100/50">Source link unavailable</span>}</div><p className="mt-3 text-sm leading-6 text-violet-50/65">Their opening: “{hook.sourceHook}”</p><p className="mt-2 text-xs text-violet-100/50">{hook.evidence}</p></article>;
      })}</div>
